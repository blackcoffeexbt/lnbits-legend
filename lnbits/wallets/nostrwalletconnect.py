...
class NostrClient:
    ...
    async def connect_to_nostrclient_ws(
        ...
        logger.info("Subscribing to websockets for nostrclient extension")
...
class NostrWalletConnectWallet(Wallet):
    def __init__(self):
        ...
        task1 = loop.create_task(
            catch_everything_and_restart(_subscribe_to_nostr_client)
        )
    ...
    async def nwc_list_paid_invoices(self, since_timestamp: int) -> List[Dict]:
        ...
        eventdata = {
            ...
            "params": {"limit": 5, "unpaid": False, "type": "incoming"},
        }
        event = self.build_encrypted_event(
            json.dumps(eventdata),
            self.secret,
        ...
        try:
            ...
            if type.upper() == "EVENT":
                ...
                if event.kind == EventKind.WALLET_CONNECT_RESPONSE:
                    logger.info("Received Wallet Connect Response")
                    ...
                    logger.info("Received Wallet Connect Info")
                    message = await self._handle_nip04_message(event)
                    logger.debug(f"Message: {message}")
                return
